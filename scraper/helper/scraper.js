...
const getBrowserAndPage = async({
    ...
    const context = await browser.newContext({
        userAgent: process.env.DEFAULT_USER_AGENT || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
    });
    ...
    await page.route('**/*', async route => {
        ...
        if (
            ...
            // Continue the route with CSP header modification
            const headers = request.headers();
            headers['Content-Security-Policy'] = "default-src 'self'; connect-src 'self' http://localhost:3000";
...
module.exports = {
    ...
    elem, // Get an element using the selector.
    input, // Input the value in the given selector.
    click, // Click on the given selector.
    doubleClick, // Double click on the given selector.
    clickCheckBox, // Click on the checkbox.
    getData, // Get the data from the given selector.
    _wait, // Wait for the given time.
    getBrowserAndPage, // Get the browser as well as the page.
